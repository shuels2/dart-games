...
- ✅ **Define helpers inline in the screenshot test file** — do NOT import a per-test-directory `_helpers.dart` (or any sibling helper file). Match the structure of every other game's screenshot test. Shared helpers under `integration_test/shared/` ARE fine — only local sibling-file imports are the hazard.
