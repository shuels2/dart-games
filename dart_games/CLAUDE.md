...
### 🧪 Testing (2837 tests total — non-UI: 2250, UI: 722 optional)
- [Test Overview](docs/testing/test-overview.md) - **2060 Flutter + 190 server + 722 UI tests**
- [Non-UI Tests](docs/testing/non-ui-tests.md) - 2250 non-UI tests (MANDATORY before builds)
...
- [Tiki Golf](docs/games/tiki-golf/) - Lilo & Stitch-styled mini-golf dart game (2-16 players)
...
cd server && dart test
```
**Required:** 100% pass rate (2250 tests total)
...
**Total: 2837 tests**
- **Flutter Non-UI Tests:** 2060 tests (100% pass rate MANDATORY)
  - API client tests: 49 (5 config + 38 client + 6 voice settings)
  - Model tests: 98 (40 core + 58 additional)
  - Model serialization tests: 142 (HorseRace 10 + TargetTag 13 + MonsterMash 13 + ReefRoyale 19 + ClockworkQuest 19 + LunarLander 12 + PiratesGrid 24 + GladiatorArena 17 + TikiGolf 15)
  - Provider tests: 74 (PlayerProvider 44 + DartboardProvider 30)
  - Provider save/restore tests: 70 (6 prior games x 7 + PiratesGrid 12 + GladiatorArena 15 + TikiGolf 13)
  - Provider game mechanics tests: 381 (HorseRace 50 + ClockworkQuest 49 + MonsterMash 44 + ReefRoyale 45 + TargetTag 45 + GladiatorArena 81 + TikiGolf 67)
...
- **UI Automation Tests:** 722 tests (optional, ask before running)
  ...
  - **Sequential (`run_ui_tests.bat`): ~843+ minutes — interactive Chrome sessions**
  - **Parallel (`run_ui_tests_parallel.bat`): ~211+ minutes — fully headless, no visible Chrome**

## Critical Reminders

### Before Any Build
✅ Run `flutter test` - ALL 2060 Flutter non-UI tests MUST pass
...
- ✅ **Define helpers FULLY inline in the screenshot test file** — both the helper interface AND its body. Do NOT import `_helpers.dart`, AND do NOT delegate to `shared/dart_throw_helpers.dart` or `shared/game_setup_helpers.dart` either. Talk directly to `package:dart_games/services/mock_scolia_api_service.dart` and the per-test `SettingsHelpers`/`UITestHelpers`/`PumpSequences`/`ProviderHelpers` shared files. The screenshot-driver web-compile path has cache hazards with helper imports that the integration_test driver doesn't share. Reference: copy the structure of `integration_test/pirates_grid/visual_validation/pirates_grid_screenshot_test.dart` line-for-line.
- ❌ NEVER use `test_driver/integration_test.dart` for screenshot tests (will hang silently on `takeScreenshot()`)
- ❌ NEVER split a screenshot test into multiple `testWidgets` blocks — the `integration_test_driver_extended` request/response protocol expects ONE test per file. Multiple `testWidgets` work under sequential `-d chrome` (in-process) but fail under parallel `-d web-server` with `AppConnectionException` / `SocketException` at `WebDriver.quit` because DWDS disconnects between test transitions. Symptom: log stops after "Debug service listening" with no "Starting application from main method", `flutter drive` crashes after ~14s.
- ❌ NEVER `import '_helpers.dart' as h;` (or any other local sibling file) from a screenshot test — same failure mode as multiple `testWidgets`: works under sequential `-d chrome` but the parallel `-d web-server` web-compile path silently mis-handles per-test-directory imports (cf. commit cea7027 / 4d1377e about new `integration_test/shared/` files being silently ignored — same cache hazard). Symptom is identical to multiple-testWidgets: hangs at "Debug service listening", `SocketException` at `WebDriver.quit` ~14s in. Inline the helpers instead.
- ❌ NEVER import `shared/dart_throw_helpers.dart` or `shared/game_setup_helpers.dart` from a screenshot test — same parallel-mode failure mode again. Even though those files live under `integration_test/shared/`, they pull in just enough additional surface to trip the screenshot-driver web-compile cache hazard. Past failure: Tiki Golf's screenshot test continued to fail in parallel even AFTER (a) consolidating to one `testWidgets` and (b) removing the `_helpers.dart` import — the third differentiator that finally fixed it was removing these two shared imports and inlining the helper BODIES (talk to `mock_scolia_api_service` directly, like Pirate's Grid / Gladiator Arena / etc.).
- ⚠️ **A SINGLE SCREENSHOT TEST FILE MUST FINISH IN UNDER 600s (10 min) END-TO-END.** The parallel UI worker (`run_ui_tests_parallel_worker.bat:253`) polls the per-test log for `'All tests passed' | 'Some tests failed' | 'Application finished' | 'Failed to compile application'` for up to 600 seconds, then kills Chrome. If the test is still running at 600s, the framework never gets to emit "All tests passed" → log shows only "Debug service listening" → `SocketException` at `WebDriver.quit` when flutter_tools tries to tear down a dead Chrome. **This is the SAME log signature as the multi-`testWidgets` / helper-import failure modes above**, so check duration before assuming it's a structural issue. **Diagnostic check:** when you see this SocketException with no test prints, look at the worker's `DURATION=` in `<game>_results.txt` — if it's near 600s, total runtime is the cause. **Fix:** split the screenshot test into multiple files (e.g. `<game>_screenshot_test.dart` for menu/solo, `<game>_screenshot_results_test.dart` for team/results). Both filenames match the runner's `findstr /i "screenshot"` driver-detection, so each gets its own 600s budget. Past failure: Tiki Golf's full screenshot test (28 captures, 2 full 9-hole rapid-completion loops) hit the 600s timeout silently until split into PARTS 1-5 + PARTS 6-10 across two files.
...
│       └── tiki-golf/               # Tiki Golf docs (8 files)
...
**Documentation Version:** 4.6 (Tiki Golf)
