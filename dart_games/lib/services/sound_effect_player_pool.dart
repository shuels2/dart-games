...
class SoundEffectPlayerPool {
  ...
  Future<void> play(SoundEffectConfig sfx) async {
    ...
    // Schedule the fade/stop timer FIRST. It runs on Dart's event loop
    // and is unaffected by whether the audio engine actually starts
    // playback. In a unit-test environment with no audio platform, this
    // still sets up correctly; in production it correctly tracks the
    // playing clip's lifetime.
    ...
    unawaited(_playOnPlayer(player, sfx));
  }

  Future<void> _playOnPlayer(AudioPlayer player, SoundEffectConfig sfx) async {
    try {
      await player.stop();
