...
if defined STUB_MODE (
    ...
    REM DIAGNOSTIC PROBE — --verbose added for tiki_golf hang investigation; revert after experiment
    start /B "" cmd /C "cd /d %_WORKTREE_PATH% && flutter drive --verbose --driver=test_driver/!_RST_DRIVER! --target=!_RST_TARGET! -d web-server --browser-name=chrome --driver-port=%_CD_PORT% --web-port=%_WEB_PORT% --dart-define=SERVER_PORT=%_SERVER_PORT% --browser-dimension=1920x1080 >> "!_RST_LOG!" 2>&1"
